...
public class DatabaseBootstrap : IDatabaseBootstrap
{
    ...
    public void Setup()
    {
        //Configurações de tipos não suportados no SQLite
        ...
        CriarViewSaldoContaCorrente(connection);
    ...
    public void CriarViewSaldoContaCorrente(SqliteConnection connection)
    {
        var view = connection.Query<string>("SELECT name FROM sqlite_master WHERE type='view' AND (name = 'vwSaldoContaCorrente')");
        var viewName = view.FirstOrDefault();
        if (!string.IsNullOrEmpty(viewName))
            return;

        connection.Execute("CREATE VIEW vwSaldoContaCorrente AS " +
                           "  SELECT " +
                           "      CC.idcontacorrente AS idcontacorrente, " +
                           "      SUM( " +
                           "         CASE " +
                           "         WHEN M.idmovimento IS NULL " +
                           "           THEN 0 " +
                           "         WHEN M.tipomovimento = 'D' " +
                           "           THEN M.VALOR * -1 " +
                           "         ELSE M.VALOR " +
                           "         END " +
                           "      ) AS Saldo " +
                           ...
                           "  LEFT " +
                           "  JOIN movimento M on M.idcontacorrente = CC.idcontacorrente " +
                           "  GROUP BY CC.idcontacorrente;");
    }
}
